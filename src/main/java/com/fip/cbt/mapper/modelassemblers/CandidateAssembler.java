...
public class CandidateAssembler implements RepresentationModelAssembler<Candidate, EntityModel<Candidate>> {
    ...
    public EntityModel<Candidate> toModel(Candidate candidate) {
        ...
        //linkTo(methodOn(FilmController.class).getAllFilms()).withSelfRel());
        //linkTo(methodOn(FilmController.class).getAllFilms()).withRel("api/v1/films"));
        //linkTo(methodOn(FilmController.class).getAllFilms()).withRel(baseURL));
    }
}
